...
- **RTL Support**: Arabic (ar) uses RTL layout with dynamic `dir` attribute
- **Translation Files**: `messages/[locale].json`

**Adding New Translations**:
1. Add keys to all `messages/*.json` files
2. Use `t('key')` in components (NEVER hardcode strings)
3. Run `npm run check-i18n` to verify completeness
4. Run `npm run check-hardcoded` to find hardcoded strings

**Important Rules**:
- ✅ All user-facing text MUST use translation keys
- ✅ Use logical CSS properties for RTL support
- ❌ NEVER hardcode English or any language text
- ❌ NEVER concatenate translated strings

**Testing**:
```bash
npm run check-i18n         # Check translation completeness
npm run check-hardcoded    # Find hardcoded strings
npm run dev                # Test language switching at localhost:3000
```
