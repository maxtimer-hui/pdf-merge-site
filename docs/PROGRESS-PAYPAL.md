...
**部署日期**: 2026-01-20
**部署平台**: Vercel
**生产环境**: https://pdf-tools-sepia.vercel.app

### 环境变量配置
| 变量名 | 值 | 模式 |
|--------|-----|------|
| `NEXT_PUBLIC_PAYPAL_CLIENT_ID` | `AXUgVnwkisScIjDxbKK-rVtpbasFLfBgzaALwuIfT18FlH575VeoAc-YqqZI-erIVzojsS4RFUNrrjdj` | Sandbox |

### 已知限制
⚠️ **PayPal 企业账户要求**
- PayPal 需要**企业账户**才能启用 Live 模式接收真实支付
- 个人账户只能使用 Sandbox 测试模式
- 当前配置为 Sandbox 模式，无法接收真实捐赠

### 错误处理
✅ 已添加 PayPal SDK 加载失败处理：
- SDK 加载失败时显示友好提示
- 不影响页面其他功能
- 用户可正常使用 PDF 工具
